let selectedDateKey = null;
    ...
    let lastMobileTap = { key: null, time: 0 };
    ...
    function installDirtyWatch(){
      ...
      document.addEventListener('click', function(e){
        ...
        if(target.closest('.weekday-btn') || target.closest('.delete-day-btn')) markDirty();
      }, true);
      updateDirtyUI();
    }
    ...
    function renderCalendar(){
      ...
      for(let day=1; day<=lastDate; day++){
        ...
        if(hasWork && !isMobileView()) html += '<button type="button" class="delete-day-btn" aria-label="근무일 삭제" onclick="deleteWorkDayFromCalendar(event, \' ' + dateKey + '\'.trim())">×</button>';
        if(holidayName) html += '<div class="holiday-badge' + (hasWork ? ' with-delete' : '') + '">' + holidayName + '</div>';
        if(hasWork){
          const rec = workRecords[dateKey];
          const night = calculateNightHours(rec.startTime, rec.endTime, rec.breakHours);
    ...
    function renderMobileAllowanceLegend(){
      ...
      box.innerHTML = '<div class="mobile-allowance-legend-title">추가수당 표시</div><div class="mobile-allowance-legend-items">' + (items || '<span class="muted">추가수당이 적용된 날짜에는 작은 원이 표시돼요.</span>') + '</div><div class="small-note"><span class="delete-hint">근무일 삭제: 같은 날짜를 빠르게 두 번 터치하면 삭제됩니다.</span></div>';
    }
    ...
    function handleCalendarClick(dateKey, year, month, day){
      ...
      if(workRecords[dateKey] && lastMobileTap.key === dateKey && now - lastMobileTap.time < 420){
        lastMobileTap = { key: null, time: 0 };
        deleteWorkDay(dateKey);
        return;
      }
      lastMobileTap = { key: dateKey, time: now };
    ...
    function deleteWorkDayFromCalendar(event, dateKey){
